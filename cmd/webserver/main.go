...
func main() {
	...
	store, closeStore, err := gtfsproxy.FileSystemGtfsStoreFromFile(settings.WorkingDirectory + "/" + settings.GtfsFilesLogFile)
	...
	server, err := gtfsproxy.NewProxyServer(store)
