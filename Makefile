...
.PHONY: all clean build release cli webserver
...
cli:
	go build -o $(BUILD_DIR)/$(PROJECT_NAME)_cli $(CLI_MAIN)

webserver:
	go build -o $(BUILD_DIR)/$(PROJECT_NAME)_webserver $(WEBSERVER_MAIN)
...
release:
	...
	@for platform in $(PLATFORMS); do \
		...
		GOOS=$$OS GOARCH=$$ARCH go build $(LDFLAGS) -o $(BUILD_DIR)/release/$(PROJECT_NAME)_$(VERSION)_$$OS-$$ARCH/$$cli_output_name $(CLI_MAIN); \
		GOOS=$$OS GOARCH=$$ARCH go build $(LDFLAGS) -o $(BUILD_DIR)/release/$(PROJECT_NAME)_$(VERSION)_$$OS-$$ARCH/$$webserver_output_name $(WEBSERVER_MAIN); \
...
help:
	...
	@echo "  make release   - Creates releases for all platforms"
